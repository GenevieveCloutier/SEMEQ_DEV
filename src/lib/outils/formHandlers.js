...
export async function connexion(event) {
	...
	log('dans le handler connexion response = ', response);

	const result = await response.json();
	log('Dans le handler connexion result = ', result);

	if (result.status === 200) {
		window.location.href = '/';
	} else if (result.status === 401) {
		log('dans le handler connexion dans le elsif data = ', JSON.parse(result.data));
...
export async function creationOrganisateur(event) {
	...
	const result = await response.json();
	if (result.type == 'failure') erreur.set(JSON.parse(result.data)[0]);
	else alert("Organisateur créé, mais aucune idee d'ou c'est partie");
