...
export const actions = {
	...
	connexionUtilisateur: async ({ cookies, request }) => {
		const data = await request.formData();
		log('api data = ', data); //*le data contient les champs de formulaire avec les infos du captcha
		try {
			const token = data.get('cf-turnstile-response'); //*On recupere le token generé par le captcha
			log('api token = ', token);
			...
			const cloudflare = await response.json(); //*L'objet renvoyé par l'api
			log('api cloudflare avant le if = ', cloudflare);
            //*qui contient un element success(true|false)
			if (cloudflare.success) {
				//*Token validé, continue avec le traitement
				log('api cloudflare = ', cloudflare);
			} else {
				//*Si le captcha ne renvoie pas de succcess, on envoie une erreur
				log('api cloudflare dans le else = ', cloudflare);
				return fail(401, { message: "Le captcha n'est pas valide" });
			}
